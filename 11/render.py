...
def render(power_level, fuel_cell, square):
    fig = plt.figure(frameon = False)

    n = power_level.shape[0]

    mx, my = fuel_cell
    power_level[my:my + 3, mx:mx + 3] *= 2;

    mx, my, sz = square.x - 1, square.y - 1, square.size
    power_level[my:my + sz, mx:mx + sz] *= 2;

    plt.axis('off')
    plt.imshow(power_level, cmap = 'hot', interpolation = 'nearest', extent = (0, n - 1) * 2)
...
render(power_level, maximum_power_fuel_cell, square)
