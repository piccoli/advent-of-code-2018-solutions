...
def main(screen):
    ...
    number_of_pots = evolve(initial_state, patterns, len(Expected) - 1, screen)

    screen.clear()
    screen.addstr(str(number_of_pots))
    screen.refresh()
...
def evolve(initial_state, patterns, num_generations, screen):
    ...
    for generation in range(num_generations):
        ...
        for i in range(2, len(current_state) - 2):
            ...
            print_state(screen, patterns, current_state, next_state, i, generation)
    ...
    return compute_total(current_state, indices)
...
def print_state(screen, patterns, cur, nxt, i, gen):
    screen.move(0, 0)
